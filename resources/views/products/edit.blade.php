...
<div class="max-w-xl mx-auto bg-white p-6 rounded-lg shadow-md">
    ...
    <form action="{{ route('products.update', $product->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="image" value="{{ old('image', $product->image) }}" class="w-full border-gray-300 rounded-lg shadow-sm border px-3 py-2" required>
        </div>

        <div class="mb-4">
            <label class="block text-gray-700 font-medium mb-2">Descrição</label>
            <textarea name="description" rows="4" class="w-full border-gray-300 rounded-lg shadow-sm border px-3 py-2" required>{{ old('description', $product->description) }}</textarea>
        </div>
